"""MASAC on the highway-merge negotiation task (Environment C).

Conditions:
  baseline       lambda = 0
  oracle         lambda = 0, listener observes the merger's intended slot
  blind          lambda = 0, listener cannot observe the merger (control)
  gapprog        hand-crafted gap-progress listener (1D progress analog)
  simple         hand-crafted speed-deviation listener
  learned_pre_prag  bounded learned listener (audience viewpoint, pre-merge
                    training window) + one RSA step -- the paper's recipe
  inforeg        Strouse-style variational I(A;M|S) (two decoders)

Usage: python train_merge.py --condition oracle --seed 0 --outdir results_merge/oracle_s0
"""
...
HANDCRAFTED = ("gapprog", "simple")


def evaluate(actor, cond, listener, seed, n_envs=64, device=torch.device("cpu"),
             listener2=None):
    env = H.HighwayMergeEnv(n_envs, oracle=(cond == "oracle"),
                            blind=(cond == "blind"), seed=seed)
    obs = env.reset()
    keys = ["r_ext", "merge_rate", "fail_rate", "harsh", "comm_rw",
            "listener_acc"]
    ...
    with torch.no_grad():
        for t in range(H.EPISODE_LEN):
            ...
            a = a.cpu()
            pre_x = env.x[:, 0].clone()
            pre_slots = env.slot_centers().clone()
            next_obs, info, done = env.step(a)
            for k in ["r_ext", "harsh"]:
                tot[k] += info[k].mean().item()
            if cond == "gapprog":
                tot["comm_rw"] += H.slot_progress_reward(env, pre_x, pre_slots).mean().item()
            elif cond == "simple":
                tot["comm_rw"] += H.simple_speed_reward(env).mean().item()
...
def main():
    ...
    p.add_argument("--condition", required=True,
                   choices=["baseline", "oracle", "blind", "gapprog", "simple",
    ...
    p.add_argument("--voi", type=float, default=0.2,
                   help="post-merge weight on R_comm (pre-merge weight is 1)")
    ...
    env = H.HighwayMergeEnv(args.n_envs, oracle=(args.condition == "oracle"),
    ...
    listener = (H.MergeListener(inputs="full" if inforeg else "act").to(device)
                if use_listener else None)
    opt_l = (torch.optim.Adam(listener.parameters(), lr=1e-3)
             if use_listener else None)
    listener2 = H.MergeListener(inputs="state").to(device) if inforeg else None
    ...
    for cycle in range(args.cycles):
        ...
        for t in range(H.EPISODE_LEN):
            ...
            pre_x = env.x[:, 0].clone()
            pre_slots = env.slot_centers().clone()
            pre_merge = 1.0 - env.merged.float()
            target = env.target
            next_obs, info, done = env.step(a)
            ...
            if args.condition in HANDCRAFTED:
                with torch.no_grad():
                    if args.condition == "gapprog":
                        rc = H.slot_progress_reward(env, pre_x, pre_slots)
                    else:
                        rc = H.simple_speed_reward(env)
                    r_comm[:, 0] = rc * (args.voi + (1 - args.voi) * pre_merge)
            ...
            for _ in range(args.updates_per_step):
                ...
                if use_listener:
                    ...
                    if prekey:
                        # pre-merge window: merger still on the ramp (lane=1)
        ...
        if (cycle + 1) % 10 == 0 or cycle == args.cycles - 1:
            ...
            print(f"[{args.condition} s{args.seed}] cyc {cycle+1}/{args.cycles} "
                  ...
                  f"fail {m['fail_rate']:.2f} comm {m['comm_rw']:.2f} "
