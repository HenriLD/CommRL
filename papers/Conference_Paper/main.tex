...
\item We evaluate the resulting reward family in a cooperative navigation task with private preferences. Hand-crafted listeners exhibit a failure mode we term \emph{miscalibrated signaling}---agents pay a large task-reward cost for signals that carry no decodable information, detectable as an intrinsic reward stuck below the uninformative level---while the learned listener is reliably optimized and yields more decodable behavior at no task cost. An oracle comparison additionally shows that the task value of intent transmission can be near zero even when intentions are private, arguing that legibility methods must be evaluated on information transmitted and task cost separately.
\end{itemize}
...
\begin{table}[t]
\centering
\caption{Final performance (mean $\pm$ s.e.m.\ over 3 seeds, averaged over the last three evaluations). $\Rext$ is the per-step team reward; Spec.\ is team specialization; $\Rcomm$ is each condition's own communicative reward (information-gain scale).}
\label{tab:final}
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{lcccc}
\toprule
\textbf{Condition} & $\Rext$ & Spec. & Probe acc. & $\Rcomm$ \\
\midrule
Baseline & $1.07\pm.03$ & $.847$ & $.406$ & --- \\
Oracle & $0.99\pm.04$ & $.841$ & $.394$ & --- \\
Exclusivity $L_0$ & $0.57\pm.04$ & $.815$ & $.395$ & $-2.39$ \\
Simple $L_0$ & $0.93\pm.06$ & $.833$ & $.393$ & $-1.33$ \\
Learned $\Ltheta$ & $1.03\pm.08$ & $.847$ & $.397$ & $-0.13$ \\
Learned + RSA & $\mathbf{1.09\pm.03}$ & $\mathbf{.853}$ & $.397$ & $-0.13$ \\
\bottomrule
\end{tabular}
\end{table}
...
The learned listener changes the picture qualitatively. Both learned conditions steadily optimize their communicative reward from $-0.65$ toward zero (Fig.~\ref{fig:comm_reward}), confirming that the co-adapted reward is learnable---by construction, it only credits distinctions that are actually present in the behavioral distribution. This calibration eliminates the destructive interference: task performance is statistically indistinguishable from the baseline ($1.03\pm.08$ and $1.09\pm.03$ vs.\ $1.07\pm.03$), with the pragmatic variant nominally best on task reward, specialization, and collision penalty. The listener's held-out accuracy climbs from chance ($1/3$) to $\approx.43$ and is still rising at the end of training (Fig.~\ref{fig:listener_acc}), with the RSA-recursed variant consistently more decodable than the purely literal one---evidence that contrastive reasoning over alternatives extracts additional signal beyond maximum-likelihood decoding. Agents thus become genuinely more legible \emph{at no task cost}, in sharp contrast to the hand-crafted listeners.

\subsection{The Oracle Ceiling}
\label{ssec:results_oracle}

An instructive negative result concerns the oracle: giving agents their teammates' true preferences does \emph{not} improve task performance ($0.99$ vs.\ $1.07$). In this environment, near-optimal coordination is achievable from positions and velocities alone---who is closest to what resolves most conflicts---so the marginal task value of intent information is small. This bounds what \emph{any} legibility method can deliver here in task reward, and reframes the meaningful comparison: among methods that add a communicative incentive, the question is not how much they gain but how little they destroy while making behavior decodable. By that standard the learned listener succeeds precisely where the hand-crafted ones fail. It also cautions against evaluating implicit communication only through task reward: the information a method transmits (Fig.~\ref{fig:listener_acc}) and its cost (Fig.~\ref{fig:task_reward}) must be measured separately.
...
\textbf{Learning the listener closes the loop.} Training $\Ltheta$ on the agents' own replay data guarantees that the common ground never demands what the policy distribution does not contain; the reward then only sharpens distinctions that already exist, which is why it optimizes smoothly and costs no task performance. Proposition~\ref{prop:mi} explains the mechanism: the learned-listener reward is a variational mutual-information bound, so policy improvement against it is information maximization---grounded, by construction, in a decodable channel. The RSA recursion contributes on top of this literal layer: it consistently improves the listener's held-out accuracy, suggesting that pragmatic contrast (scoring an action against foregone alternatives) shapes behavior in ways that pure maximum-likelihood decoding does not capture. The combination---a learned common ground with pragmatic reasoning over it---attained the (nominally) best task reward, specialization, and decodability of any condition, though its task-reward advantage over the baseline lies within seed noise.
