...
\item We map the boundary of the method by sweeping what early intent is worth and what signaling costs, obtaining a proportionality law---$\text{gain}\approx0.51\times\text{premium}$ across three environments, three partner speeds and two signal costs---together with four distinct failure modes (no premium, no channel, partner too slow to act, signal too costly) that make ``when pragmatism pays'' a checkable condition rather than an empirical accident.
...
Listener is bounded & Learned $\Ltheta$ ($97\%$) & n.s. \\
Listener is calibrated & Exclusivity $L_0$ & $0.16\times$ \\
Weight is moderate & $\lambda=1.0$ & $-0.24$ \\
Evaluated at convergence & $150$ cycles & sign flips \\
\midrule
\textit{All hold} & \textit{3 environments} & $\mathbf{0.51\times}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{The boundary map. \textbf{Left:} across every setting with a resolvable oracle premium---three environments, three partner speeds, two signal costs---the pragmatic gain is about half the premium (dashed fit; dotted line is the oracle, i.e.\ capturing all of it). Settings where the partner is too slow to act on a signal, or where signaling is too expensive to emit, fall below the line despite large premiums. \textbf{Right:} where no premium is resolvable the gain is not significant, and where the channel is severed the reward is pure cost. Bars are $\pm$ s.e.m.\ over 5--10 seeds.}
\label{fig:boundary}
\end{figure}
...
\caption{The pre-convergence hazard: the estimated fraction of the premium closed, as a function of the training budget at which the comparison is made (bootstrap $95\%$ bands, 10 seeds). The progress listener would be judged \emph{harmful} for the first $0.5$M steps and null at our early evaluation budget (vertical line); the ordering among conditions stabilizes only past ${\sim}1$M steps.}
\label{fig:budget}
\end{figure}
