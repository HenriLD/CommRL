...
\end{equation}
and $L^* = L_{N_r}$. Because the alternative set is sampled, $\Rcomm$ is a stochastic reward whose sampling noise the critic averages over. When alternatives are drawn from the current policy---or when $L_0$ itself co-adapts, as with the learned listener below---$\Rcomm$ is additionally policy-dependent and hence non-stationary; the Bellman analysis of Sec.~\ref{ssec:bellman} then holds per reward snapshot, as in other co-adaptive intrinsic rewards \cite{eysenbach2019diversity}. The choice of $L_0$ is the framework's most consequential free parameter, and we study four instantiations.
...
Three remarks locate Proposition~\ref{prop:mi} precisely. First, it holds for \emph{any} conditional distribution over the same conditioning variables: the instantaneous hand-crafted listeners of (i)--(ii) also yield variational lower bounds on the same mutual information, only looser ones (the filter listener, which conditions on the action history, instead bounds the larger quantity $I(M; S_{1:t}, A_{1:t})$ and can exceed the instantaneous information). What distinguishes the learned listener is not being a bound but being the \emph{tightest} bound available from the data; the empirical calibration results of Sec.~\ref{ssec:results_learned} are exactly bound-tightness. Second, the connection to prior objectives is close but not exact: the skill discriminator of DIAYN \cite{eysenbach2019diversity} maximizes a bound of this form, while the cooperative information regularizer of Strouse et al.~\cite{strouse2018learning} targets the \emph{conditional} information $I(A;G|S)$---which differs from $I(M;S,A)$ by the state-borne term $I(M;S)$---and the positive-signaling bias of Eccles et al.~\cite{eccles2019biases} shapes an explicit message channel with entropy targets rather than a variational decoder. Within our framework all three are forms of the \emph{literal} ($N_r=0$) level of pragmatic reasoning with a learned common ground. Third, the RSA recursion scores an action not by how much information it carries in isolation but by how it contrasts with the alternatives the agent declined \cite{goodman2016pragmatic,zaslavsky2021rate}; since the recursion moves $L^*$ away from the maximum-likelihood fit, it loosens the expected-reward bound of Proposition~\ref{prop:mi} whenever that fit is at the posterior (with finite data the direction is not guaranteed, but tightening is not its purpose)---its value, where we find any, is as a \emph{shaping signal}, not as an information estimate (Sec.~\ref{ssec:results_bounded}).

Proposition~\ref{prop:mi} also yields formal content for the \emph{vocabulary} clause of the audience-matching principle. Model the intended reader as computing its behavior from a feature class $\Phi$ of statistics of the observable history, and say a listener is \emph{vocabulary-restricted to $\Phi$} if it factors as $L(m|s,a) = g(\varphi(s,a))$, $\varphi \in \Phi$. Applying Proposition~\ref{prop:mi} to the pushforward channel $(M, \varphi(S,A))$ and the data-processing inequality gives: the expected reward of a vocabulary-restricted listener lower-bounds $I(M; \varphi(S_t,A_t)) \leq I(M; S_t, A_t)$---it can only pay for codes the reader can extract. Two consequences organize our results: a vocabulary-restricted listener's saturation certifies that the \emph{reader-visible} channel carries the meaning (success, not pathology), while an unrestricted listener can saturate through reader-invisible information; and our listener family sits at measurable points of the restriction spectrum (progress/filter fully restricted to motion statistics; viewpoint-bounded learned restricted in $\varphi$ but free in $g$; full-context unrestricted), with empirical gains tracking the restriction ordering exactly.
...
Fix a listener $L$ and a policy $\pi_\phi$ with occupancy measure $\rho_\pi$, and suppose the reward is clipped below at $-B$ (i.e., $\Rcomm = \max(\log L(m_t|s,a), -B - \log|\mathcal{M}|) + \log|\mathcal{M}|$). If $\;\rho_\pi\!\left(L(m_t|s,a) \geq 1-\epsilon\right) \geq 1-\delta$, then for any score-function (policy-gradient) estimator with baseline $b = \mathbb{E}[\Rcomm]$,
\begin{equation}
...
\end{equation}
which is $O(\epsilon + \delta B)$ per step; over a discounted horizon the constant is scaled by at most $1/(1-\gamma)$.
\end{proposition}

\begin{proof}
On the $(1-\delta)$-mass, $\Rcomm \in [\log(1-\epsilon) + \log|\mathcal{M}|,\, \log|\mathcal{M}|]$, an interval of width $-\log(1-\epsilon)$; on the remaining $\delta$-mass the clipped reward deviates from $b$ by at most $B + \log|\mathcal{M}|$. The estimator $\mathbb{E}[\nabla \log \pi \cdot (\Rcomm - b)]$ is bounded by the expected deviation of the reward from $b$; $-\log(1-\epsilon) = \epsilon + O(\epsilon^2)$.
\end{proof}

Two scope notes. The high-probability form is the one our experiments instantiate: a Gaussian policy has unbounded support, so a pointwise bound would be vacuous, but the measured decode accuracies (Table~\ref{tab:bounded}) are exactly estimates of $1-\delta$ at small $\epsilon$. And our agents are trained with reparameterized (SAC) gradients rather than score-function estimators; there the same interval argument bounds the spread of the reward entering the critic targets, so a saturated listener shifts $Q$ by a near-constant that policy improvement cancels. We therefore use Proposition~\ref{prop:saturation} as the mechanism \emph{consistent with} the observed inertness rather than a quantitative prediction. It formalizes the saturation direction of the \emph{audience-matching principle} developed in Sec.~\ref{sec:discussion}---a listener that decodes everything demands nothing---while the principle's second clause (crediting the right vocabulary) remains empirical. Note the antecedent is a property of the listener \emph{under the policy's occupancy}---a listener can saturate either by being given context that reveals the meaning regardless of the action (as our full-context learned listener does) or by co-adapting fast enough to decode whatever the current policy emits (as our viewpoint-bounded one does, Sec.~\ref{ssec:results_bounded}).
...
\begin{split}
    Q_t(s_t, a_t) ={}& \Rtotal(s_t, a_t) \,+ \\
                     & \gamma\, \mathbb{E}_{s_{t+1}\sim P(\cdot|s_t,a_t)}\Big[\AlphaRL \log \!\int \exp\big(\tfrac{1}{\AlphaRL}Q(s_{t+1}, a')\big)\, \mathrm{d}a'\Big],
\end{split}
\end{equation}
...
All agents are trained with multi-agent Soft Actor-Critic (MASAC)---parameter-shared Gaussian actors (roles distinguished by a one-hot flag) and a centralized twin critic over joint observations and actions \cite{lowe2017multi}, with automatic entropy tuning---the natural multi-agent extension of the Max-Ent framework our theory is built on. The critic is trained on the scalar team reward with $\lambda\Rcomm$ added; $\Rcomm$ is computed from the scout's action and paid into that team scalar, so ``only the scout receives $\Rcomm$'' means the reward is a function of the scout's behavior, credited through the shared critic. Conditions differ only in reward and observation content: \textbf{baseline} ($\lambda=0$); \textbf{oracle} ($\lambda=0$, private states shared through observations); the four hand-crafted listeners of Section~\ref{ssec:listeners}---\textbf{simple}, \textbf{exclusivity} (both with one RSA step over $K=64$ sampled alternatives), \textbf{progress}, and \textbf{filter}; and the two learned ones, \textbf{learned} ($N_r=0$) and \textbf{learned + RSA} (one recursion step over 16 alternatives). $\Ltheta$ is a 2-layer MLP trained by cross-entropy on replayed transitions with the private state masked from its input. In Environment A we add \textbf{ear} conditions that append the listener's posterior to the supporter's observation at execution (Section~\ref{ssec:results_scout}), \textbf{bounded learned} conditions that restrict the learned listener's inputs and training window to what the audience actually uses (Section~\ref{ssec:results_bounded}), two \textbf{external baselines}---information regularization in the style of Strouse et al.~\cite{strouse2018learning} (reward $\log q_1(m|s,a) - \log q_0(m|s)$ with two replay-trained decoders) and a partner-belief reward in the style of Tian et al.~\cite{tian2020implicit} (a decoder over the supporter's own observation)--- and a value-of-information weighting that scales $\Rcomm$ by the supporter's decision relevance (full weight before the scout's pickup, down-weighted after). Environment B uses $\lambda=0.1$ over 400 training cycles at 5 seeds (its exploratory $\lambda$ sweep at 150 cycles, labeled as such); Environment A uses $\lambda=0.3$ over 400 cycles, with the headline conditions run at 10 seeds and the rest at 3--16 as noted per table. We disclose hyperparameter provenance: $\lambda=0.3$ and the VOI weight $0.2$ were selected in preliminary short-budget sweeps on the standard scout-support task itself, then frozen; the transfers to the five-meaning, minefield, and boundary-sweep settings are therefore untuned, while the standard-task numbers are reported on the environment where the two knobs were chosen. Full hyperparameters are in Table~\ref{tab:hyperparams}, and the launch specifications in the released code determine every run exactly.
...
\textbf{Motion-grounded hand-crafted listeners close most of the recoverable gap.} The strongest conditions are the simplest: the progress listener closes $70\%$ of the oracle premium ($0.924\pm.008$, Welch $t=6.2$ vs.\ baseline), the filter listener $67\%$ ($t=6.5$), and even the plain cosine listener $62\%$ ($t=5.0$)---through the speaker-side reward alone, with the largest commitment gains of any condition ($.857$--$.861$) and visibly more legible scouts (post-hoc decoder accuracy $.61$ vs.\ $.54$; time-resolved in Fig.~\ref{fig:info_profile}). The mechanism is direct: these listeners only credit \emph{overtly} geometric signals---motion biased toward the true target---and overt signals are exactly what the supporter's own policy network can read from raw observations and motion history. No dedicated receiver machinery is needed; the channel is the behavior itself. The exception among the hand-crafted family proves the calibration rule: the exclusivity listener, whose margin demand is geometrically unsatisfiable here as in Environment B, is unstable across seeds ($0.795\pm.081$ at $n{=}8$; individual seeds range from baseline-level to collapsed)---an early $n{=}3$ estimate had suggested a positive $16\%$ closure, which the extension eliminated.
...
\textbf{Robustness and grounding.} At the converged budget the progress recipe is insensitive to its two knobs across the moderate range: $0.921$--$0.925$ for $\lambda \in \{0.1, 0.3\}$ with VOI weighting on either setting that includes it ($0.895$ at $\lambda{=}0.1$ with uniform weighting; 3 seeds per new cell), so the flagship $70\%$ closure is a plateau, not a tuned point. The plateau has a cliff: $\lambda=0.6$ is destructive for the progress reward ($0.450\pm.203$; individual seeds collapse), while the same $\lambda$ is the \emph{optimum} for the bounded learned-pragmatic reward (Sec.~\ref{ssec:results_bounded})---the safe weight range scales inversely with the reward's magnitude, which is itself an argument for bounded rewards. A parallel surface measured on the learned-ear condition is flat ($0.83$--$0.88$, $\lambda \in [0.1, 0.6]$) until $\lambda=1$ ($0.61$); the weighting effects that mattered mid-training (Sec.~\ref{ssec:results_dynamics}) wash out asymptotically. Zero-shot cross-play---pairing scouts and supporters from independently trained seeds---retains $92$--$94\%$ of self-play reward in every condition, no worse than the baseline's own cross-seed transfer; cross-paired filter teams ($0.844$) still match baseline \emph{self}-play. The emergent protocols are thus grounded in task geometry rather than seed-specific convention, addressing the fragility concern raised by the zero-shot-coordination literature \cite{obl}. Finally, at $16\times$ the parameter count (hidden width $1024$, 3 seeds) the qualitative ordering is unchanged---learned $\approx$ baseline $<$ ear $<$ oracle---though larger networks shrink the premium itself ($0.945$ vs.\ $1.008$) by coordinating better from raw observations.
...
Weight matches reward scale & $\lambda{=}1.0$ (learned); $\lambda{\ge}0.6$ (prog.) & $\le 0$ \\
...
Exclusivity $L_0$ ($n{=}8$) & $0.795\pm.081$ & n.s. & $.850$ & $-1.65$ \\
...
Learned + RSA ($n{=}8$) & $0.863\pm.008$ & $16\%$ & $.856$ & $+1.03$ \\
\midrule
Info-reg.\ \cite{strouse2018learning} ($n{=}8$) & $0.883\pm.011$ & $34\%$ & $.856$ & --- \\
Partner belief \cite{tian2020implicit} ($n{=}8$) & $0.852\pm.016$ & $6\%$ & $.856$ & --- \\
\midrule
...
\caption{The same Environment A evaluation episode under three conditions (converged policies, seed 0; the episode was selected, from 60 candidates, to maximize the baseline-vs-recipe commitment-time difference---it illustrates the mechanism, the statistics are in Table~\ref{tab:scout}). Squares are sites (the true target filled); the black cross and dashed circle mark the pickup waypoint the scout must visit first. Trajectories darken with time (dot every 5 steps; dense dots = slow motion); open circle = start, star = end, diamond = the supporter's commitment moment. The baseline supporter hesitates mid-arena (stacked dots) and commits only at $t{=}17$; the pragmatically rewarded supporter commits at $t{=}1$ and drives straight to the site, matching the oracle's behavior---early commitment bought by the scout's legible first leg.}
...
\begin{table}[t]
\centering
\caption{Environment B at the converged budget (400 cycles; mean $\pm$ s.e.m.\ over 5 seeds, averaged over the last three evaluations). $\Rext$ is the per-step team reward; Spec.\ is team specialization; $\Rcomm$ is each condition's own communicative reward (information-gain scale); Post-hoc is the decoding accuracy of a fresh listener trained on the final policy's rollouts (chance $=.33$).}
\label{tab:final}
...
\midrule
Baseline & $1.86\pm.06$ & $.859$ & --- & $.665\pm.007$ \\
Oracle & $1.93\pm.06$ & $.862$ & --- & $.672\pm.004$ \\
\midrule
Simple $L_0$ & $1.81\pm.05$ & $.849$ & $-0.92$ & $.684\pm.005$ \\
Exclusivity $L_0$ & $1.69\pm.05$ & $.832$ & $-1.83$ & $.700\pm.008$ \\
\midrule
Learned $\Ltheta$ & $\mathbf{1.93\pm.06}$ & $.847$ & $+0.21$ & $.672\pm.007$ \\
Learned + RSA & $1.87\pm.02$ & $.837$ & $\mathbf{+0.22}$ & $.684\pm.004$ \\
\bottomrule
\end{tabular}
\end{table}
...
The exclusivity listener exhibits an instructive failure mode. The composite reward is clearly operative, yet the agent never comes close to optimizing the communicative reward itself: $\Rcomm$ plateaus at $-1.83$ (Fig.~\ref{fig:comm_reward}), \emph{below} the uninformative level of zero---the pragmatic listener built on this common ground assigns the true meaning \emph{less} probability than a uniform guess. The exclusivity score (Eq.~\ref{eq:exclusivity}) demands that an action align with the true target's direction while misaligning with every competing direction; when landmarks subtend similar bearings this margin is geometrically unsatisfiable, so the agent pays a permanent tax for a signal that cannot be produced. At the converged budget the tax is a $9\%$ task-reward loss ($1.69$ vs.\ $1.86$, $t=2.3$, $p\approx.05$ uncorrected---suggestive) with lower specialization ($.832$ vs.\ $.859$); we note candidly that our earlier 150-cycle evaluation put this same tax at $46\%$, an overestimate produced by exactly the pre-convergence hazard of Sec.~\ref{ssec:results_dynamics}---miscalibrated rewards slow learning dramatically, so their harm looks catastrophic at budgets where the baseline is still climbing. The simple cosine listener, which removes the exclusivity margin, is correspondingly less harmful ($1.81$, n.s.) with a better---but still negative---$\Rcomm$ ($-0.92$). We call this failure mode \emph{miscalibrated signaling}: a hand-crafted common ground rewards behavior the policy cannot realize and an observer cannot decode, converting the intrinsic reward into interference. Its diagnostic requires no extra machinery: an intrinsic reward persistently below the uninformative level.

\subsubsection{Learned Listeners: Calibrated Legibility}
\label{ssec:results_learned}

The learned listener changes the picture qualitatively. Both learned conditions steadily optimize their communicative reward through zero to genuinely \emph{positive} information gain at convergence ($+0.21$ and $+0.22$; Fig.~\ref{fig:comm_reward}), confirming that the co-adapted reward is learnable---by construction, it only credits distinctions actually present in the behavioral distribution. This calibration eliminates the destructive interference: task performance is statistically indistinguishable from the baseline ($1.93\pm.06$ and $1.87\pm.02$ vs.\ $1.86\pm.06$), nominally at or above it. The co-adapting listener's converged accuracy separates the levels cleanly: $.644$ for the literal listener versus $.690$ with the RSA recursion---contrastive reasoning over alternatives extracts decodable signal that maximum-likelihood decoding alone does not, the same directional effect as in Environment A, here visible in the channel itself rather than in task reward (which the near-zero premium caps).

\subsubsection{How Much Was There to Say? Oracles and Post-Hoc Probes}
\label{ssec:results_oracle}

Two further measurements put the above in perspective. First, the oracle: giving agents their teammates' true preferences buys almost nothing ($1.93$ vs.\ $1.86$; premium $0.07\pm.08$, n.s.)---near-optimal coordination is achievable from positions and velocities alone, so the marginal task value of intent information is small. (At the 150-cycle budget the oracle had appeared \emph{below} baseline, an artifact of comparing learners mid-climb; at convergence the premium is properly non-negative and near zero, which is what makes this environment the control.) Second, the post-hoc probes: a fresh listener trained on any condition's converged behavior decodes intentions at $.665$--$.700$ accuracy (Table~\ref{tab:final})---tightly clustered, because acting on one's preference is extrinsically rewarded and converged behavior is already informative. The one deviation is instructive: the \emph{exclusivity} condition's behavior is the most decodable of all ($.700\pm.008$ vs.\ baseline $.665\pm.007$, $t=3.2$, uncorrected)---its costly sweeps do carry intent to an unconstrained decoder---yet its own margin-demanding listener never credits them ($\Rcomm=-1.83$). Miscalibration is a mismatch of vocabulary, not an absence of signal: the agent pays for signaling its own common ground cannot hear, which sharpens the case for either learning the listener or hand-crafting it in the audience's actual vocabulary.

Together these results reframe the evaluation of implicit-communication methods: task reward alone conflates the cost of signaling with its value, and a method should be judged on (a) whether its intrinsic reward is achievable (calibration), (b) how much decodable information the resulting behavior carries (post-hoc probes), and (c) what it pays for that information (task-reward gap). On calibration and task cost the learned listener dominates the hand-crafted ones; on post-hoc decodability the conditions tie ($.58$--$.60$, n.s.), consistent with this environment's near-zero premium.

\subsubsection{The Weight $\lambda$: Dial or Landmine?}
\label{ssec:results_lambda}

We sweep the communicative weight for both listener families in Environment B (an exploratory sweep: 2 seeds per new point at the 150-cycle budget, so we read it for its qualitative shape only; Fig.~\ref{fig:lambda_ablation}). With the learned listener, $\lambda$ behaves like a dial along an information-performance frontier: $\lambda=0.1$ is free ($\Rext=1.03$, $\Rcomm=-0.13$); $\lambda=0.3$ buys near-saturated clarity ($\Rcomm=-0.05$) for a modest task cost ($\Rext=0.84$); and $\lambda=1.0$ pushes the communicative reward to genuinely \emph{positive} information gain ($\Rcomm=+0.62$)---behavior more informative than task-optimal motion---at a large task cost ($\Rext=-0.80$) as agents privilege signaling over covering landmarks. With the exclusivity listener, no operating point buys information: at $\lambda=0.03$ the reward is simply ignored ($\Rcomm$ stuck at $-2.6$, task performance at baseline level), and at $\lambda=0.1$ it destroys 46\% of task reward with $\Rcomm$ still at $-2.4$. Calibration is precisely what turns $\lambda$ from a landmine into a dial.
...
Michele Persiani and Thomas Hellstr̦m,
...
\newblock in {\em Proceedings of the AAAI Conference on Artificial Intelligence}, 2020, pp. 7261--7268.
...
\newblock in {\em Advances in Neural Information Processing Systems}, 2020, vol.~33.
...
\newblock in {\em ACM/IEEE International Conference on Human-Robot Interaction (HRI)}, 2017, pp. 283--292.

\bibitem{monroe2015learning}
Will Monroe and Christopher Potts,
\newblock ``Learning in the rational speech acts model,''
\newblock in {\em Proceedings of the 20th Amsterdam Colloquium}, 2015.
