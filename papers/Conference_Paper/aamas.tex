...
Table~\ref{tab:scout} and Fig.~\ref{fig:listeners} summarize Environment A at the converged budget. The oracle premium---the irreducible value of early intent information---is $0.112$ per step ($0.957$ vs.\ $0.845$; commitment $.880$ vs.\ $.851$).

\textbf{Motion-grounded hand-crafted listeners close most of the gap.} The strongest conditions are the simplest: through the speaker-side reward alone, progress closes $70\%$ (Welch $t=6.2$), filter $67\%$ ($t=6.5$), plain cosine $62\%$ ($t=5.0$), with the largest commitment gains ($.857$--$.861$) and more legible scouts (fixed geometric-probe accuracy $.61$ vs.\ $.54$, confirmed by a fresh decoder external to every training condition). They credit only \emph{overtly} geometric signals---exactly what the supporter's policy network reads from raw observations; the channel is the behavior itself. The exception proves the calibration rule: the exclusivity listener, whose margin demand is geometrically unsatisfiable, is unstable across seeds (a $1$-in-$8$ collapse rate: seven seeds in $0.84$--$0.93$, one at $0.23$; an early $n{=}3$ estimate of $+16\%$ closure was eliminated by extension to $n{=}8$).
...
If the learned listener fails because it is too strong, audience matching predicts a remedy: weaken it toward the intended reader. A \emph{viewpoint-bounded} listener sees only site bearings and the action---what the audience uses to interpret the utterance---removing the contextual shortcut; a \emph{window-bounded} variant additionally trains only on pre-commitment steps, so decode accuracy cannot be earned from the post-commitment leg (Fig.~\ref{fig:listeners}).
...
Why the residual gap to the hand-crafted family ($27\%$ vs.\ $70\%$)? On commitment the recursed listener has caught up ($.8606\pm.0007$, matching progress; the gain over baseline's $.8508\pm.0013$ is $t\approx6.5$, headline-grade on its own), but a co-adapting listener stays satisfied by \emph{any} decodable regularity, including idiosyncratic codes the co-training supporter tracks imperfectly.

\textbf{The inverse-planning listener closes the gap.} That failure motivates the construction deferred in Sec.~\ref{ssec:listeners}: freeze a non-communicative baseline and Bayes-invert it. The IPL resolves the residual outright with no hand-crafted geometry---its literal form closes $55\%$ ($t=3.6$), and one RSA step reaches $66\%$ at $\lambda{=}0.3$ ($t=6.3$) and $\mathbf{70\%}$ at $\lambda{=}0.6$ ($t=5.8$), \emph{equal to the progress listener} with commitment $.862$--$.864$ exceeding it. Both recursed points clear the $t\ge4$ bar, the first learned-family results here to do so, and $66\%/70\%$ is a plateau, not a selected optimum: these are the only IPL runs made, so no selection/confirmation split was needed. It satisfies both clauses \emph{as instantiated}: frozen, hence unable to chase the policy (clause a, Proposition~\ref{prop:saturation}), and anchored in task-competent, reader-visible motion (clause b, Proposition~\ref{prop:vocab}). What the bounded listeners lacked was never hand-crafted geometry but a \emph{non-co-adapting, competence-grounded} literal listener. The recipe for any goal-conditioned task: train without communication, freeze, Bayes-invert at soft rationality, add the recursion.
...
\textbf{An empirical regularity, stated with its scope.} Wherever the premium is resolvable (point estimate $>3\times$ its s.e., fixed before analysis), the recipe captures a roughly constant fraction: an origin-constrained fit over all seven resolvable settings gives $\text{gain} \approx 0.51 \times \text{premium}$ (bootstrap $95\%$ CI $[0.42, 0.59]$). Two caveats. The two failure-mode settings below (ratio $0.37$) have the \emph{largest} premiums and dominate the fit; the remaining five give $0.68\times$ (CI $[0.57,0.79]$, ratios $0.59$--$0.82$), but that classification is post hoc, so $0.68$ is exploratory and $0.51$ primary. And gain and premium share each setting's baseline estimate, so their errors correlate; the bootstrap resamples seeds within settings. All settings come from one environment family and one algorithm: a regularity of that family, not a law, falsifiable by a resolvable-premium, healthy-channel environment with capture outside the fitted range.
...
The same discipline applies to our own terminal budget, and is why we report a \emph{ratio} rather than a difference. At 400 cycles the oracle has plateaued (terminal slope $+0.006$ reward per 100 cycles, $t{=}0.4$) but the baseline is still improving ($+0.054$, $t{=}3.9$), so the premium is still contracting: measured every ten cycles from 320 to 400 it falls from $0.149$ to $0.100$. The closure fraction is invariant over that window (progress: $.727, .728, .643, .753, .749, .752, .726, .666, .732$; no trend), because numerator and denominator contract together. Absolute rewards at this budget are therefore provisional, while the closure fractions we report throughout are stable with respect to it.
