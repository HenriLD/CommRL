...
Why the residual gap to the hand-crafted family ($35\%$ vs.\ $70\%$)? On commitment the recursed listener has caught up ($.861$), but the learned listener remains satisfied by \emph{any} decodable regularity, including idiosyncratic codes the co-training supporter tracks imperfectly; the progress listener credits only monotone geometric approach, which anything watching raw motion can read. The constructive recipe for learned reward listeners: bound the inputs to the audience's viewpoint, train on the decision window, add the pragmatic recursion, and calibrate the weight against the reward's gradient-carrying variation.
