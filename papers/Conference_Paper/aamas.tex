...
\caption{The meaning-space axis. Across discrete labels, a continuous circle, and a self-assigned latent, the literal listener stays at the baseline and RSA recursion lifts it, but only the inverse-planning listener in the discrete regime clears the $t{\ge}4$ headline bar (filled star). Whiskers are $\pm1$ s.e.m.\ as a fraction of each regime's oracle premium.}
...
\caption{Two separable dials in the self-assigned regime. \textbf{Left:} the legibility reward raises $I(Z;\text{behavior})$ across conditions while $I(Z;\text{private})$ stays flat below the critic ceiling. \textbf{Right:} stronger compression ($\beta$) prunes decoded noise leakage from $z$ while the true bearing remains fully decodable.}
