...
\caption{Two separable dials in the self-assigned regime. \textbf{Left} (each strip on its own scale): the legibility reward raises $I(Z;\text{behaviour})$ ($t{\approx}4.6$) while $I(Z;\text{private})$ stays flat, well below the $K{=}255$ critic ceiling. \textbf{Right:} stronger compression ($\beta$) prunes decoded noise leakage from $z$ while the true bearing stays fully decodable.}
